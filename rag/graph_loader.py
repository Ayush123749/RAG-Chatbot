...
from rag.config import GRAPHML_PATH, NODES_CSV_PATH, EDGES_CSV_PATH
...
class GraphStore:
    ...
    def search_by_text(self, query, limit=20):
        ...
        scored = []
        for nid, node in self.nodes_index.items():
            score = 0
            text = f"{nid} {node.get('description', '')} {node.get('entity_type', '')}".lower()
