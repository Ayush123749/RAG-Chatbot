from rag.config import TOP_K_GRAPH_RESULTS, TOP_K_TEXT_RESULTS, CHATBOT_MODEL, CHATBOT_TEMPERATURE, CHATBOT_MAX_TOKENS, OPENAI_API_KEY, OPENAI_BASE_URL
...
class RAGChatbot:
    ...
    def _two_pass_retrieve(self, query):
        ...
        stop_words = {'tell', 'me', 'about', 'what', 'is', 'the', 'a', 'an', 'of', 'in', 'for', 
        ...
        for node in first_pass:
            score = 0
            text = f"{node.get('entity_id', '')} {node.get('description', '')} {node.get('entity_type', '')}".lower()
            ...
            neighbors = self.graph.get_neighbors(node.get('entity_id', ''), limit=5)
            score += min(len(neighbors), 10)
            ...
            for preamble in preambles:
                if cleaned.startswith(preamble):
                    ...
                    if cleaned and cleaned[0].islower():
                        cleaned = cleaned[0].upper() + cleaned[1:]
                    break
            answer = self._clean_llm_output(cleaned)
        else:
            answer = "this context is not available in the knowledge graph of rel19"
